...
import { Award, CheckCircle2, Clock, Gift, ShieldCheck, Star, Trophy, Users } from "lucide-react";
import { useState } from "react";
import { useToast } from "@/hooks/use-toast";

// NCSC milestone ages and corresponding rewards
const MILESTONES = [
  { age: 80,  label: "80th Birthday",    amount: 10000,  color: "from-amber-400 to-yellow-500",   badge: "bg-amber-500/15 text-amber-400 border-amber-500/30",   icon: "🏅" },
  { age: 85,  label: "85th Birthday",    amount: 10000,  color: "from-orange-400 to-amber-500",   badge: "bg-orange-500/15 text-orange-400 border-orange-500/30", icon: "🥈" },
  { age: 90,  label: "90th Birthday",    amount: 10000,  color: "from-rose-400 to-pink-500",      badge: "bg-rose-500/15 text-rose-400 border-rose-500/30",       icon: "🥇" },
  { age: 95,  label: "95th Birthday",    amount: 10000,  color: "from-purple-400 to-violet-500",  badge: "bg-purple-500/15 text-purple-400 border-purple-500/30", icon: "💎" },
  { age: 100, label: "Centenarian (ECA)", amount: 100000, color: "from-red-500 to-rose-600",       badge: "bg-red-500/15 text-red-400 border-red-500/30",          icon: "👑" },
];
...
const NCSC = () => {
  ...
  const pendingPayouts = eligibleSeniors.reduce((count, senior: any) => {
    MILESTONES.forEach((m) => {
      if (senior.currentAge >= m.age) {
        const paid = payouts.some((p) => p.senior_id === senior.id && p.milestone_age === m.age);
        if (!paid) count++;
      }
    });
    return count;
  ...
  const getSeniorPayouts = (seniorId: string) =>
    payouts.filter((p) => p.senior_id === seniorId);

  const isPaid = (seniorId: string, milestoneAge: number) =>
    payouts.some((p) => p.senior_id === seniorId && p.milestone_age === milestoneAge);
            ...
            <div key={senior.id}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${allPaid ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" : "bg-amber-500/15 text-amber-400 border-amber-500/30"}`}>
              ...
              <div className="px-4 pb-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                {applicableMilestones.map((m) => {
                  ...
                  const isEligibleNow = senior.currentAge >= m.age;
                  ...
                  return (
                    <div key={m.age}
                      ...
                      onClick={() => {
                        if (!paid && isEligibleNow) {
                          setSelectedSenior(senior);
                          setPayoutForm({ milestoneAge: m.age, givenBy: "", remarks: "" });
                        }
                      }}
                    >
                      ...
                      {paid ? (
                        <div className="flex items-center justify-center gap-1 text-emerald-400">
                          ...
                          <span className="text-[10px] font-medium">Given</span>
                        </div>
                      ) : isEligibleNow ? (
          ...
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4 animate-fade-in">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {selectedSenior.first_name} {selectedSenior.last_name} · Age {payoutForm.milestoneAge} milestone
                </p>
              </div>
            </div>
            ...
            <div className="bg-muted/40 rounded-xl p-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-foreground">{getMilestone(payoutForm.milestoneAge)?.label}</span>
              </div>
              <div className="flex justify-between mt-1">
                <span className="text-muted-foreground">Amount</span>
                <span className="font-bold text-amber-400">₱{getMilestone(payoutForm.milestoneAge)?.amount.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  className="w-full bg-muted/50 border border-border rounded-xl px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 resize-none"
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                onClick={() => { setPayoutForm(null); setSelectedSenior(null); }}
              >
                Cancel
              </button>
              <button
                className="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90 disabled:opacity-50"
                style={{ background: "linear-gradient(135deg, hsl(38,85%,48%), hsl(6,65%,42%))" }}
                ...
                onClick={() => {
                  ...
                  addPayout.mutate({
                    ...
                    milestone_age: payoutForm.milestoneAge,
                    amount: getMilestone(payoutForm.milestoneAge)!.amount,
                    given_by: payoutForm.givenBy,
